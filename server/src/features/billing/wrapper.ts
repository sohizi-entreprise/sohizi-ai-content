 ...
 */
function combineSignals(timeoutSignal: AbortSignal, external?: AbortSignal): {
  signal: AbortSignal
  cleanup: () => void
} {
...
export function withBilling<TInput, TOutput>(
  ...
  return async function callBillable(input: TInput, ctx: WithBillingCallContext): Promise<TOutput> {
    ...
    try {
      ...
      combined.cleanup()
      await safeRefund(billing, reservation.id, timedOut ? 'timeout' : 'execute-error')
      if (timedOut) {
...
export function withBillingAsync<TInput, TJobHandle>(
  ...
  return async function startBillable(
    ...
    try {
      ...
      clearTimeout(timeout)
      combined.cleanup()
      await safeRefund(billing, reservation.id, timedOut ? 'submit-timeout' : 'submit-error')
      if (timedOut) throw new BillingTimeoutError(billable.operation, billable.timeoutMs)
      throw err
    }
    clearTimeout(timeout)
    combined.cleanup()
...
export function withBillingStream<TInput, TChunk>(
  ...
  return async function* callBillable(
    ...
    try {
      ...
      if (!settled) {
        await safeRefund(billing, reservation.id, timedOut ? 'stream-timeout' : 'stream-error')
      }
      if (timedOut) {
        throw new BillingTimeoutError(billable.operation, billable.timeoutMs)
      }
...
async function safeRefund(billing: BillingService, reservationId: string, reason: string) {
  try {
    ...
    console.error('[billing] refund after failure also failed', reservationId, err)
  }
}
