...
import { db } from '@/db'
import { organization, billingReservations, billingLedger, organizationWallets } from '@/db/schema'
...
async function countLedger(orgId: string, kind?: string): Promise<number> {
  const rows = kind
    ? await db
        .select({ id: billingLedger.id })
        .from(billingLedger)
        .where(and(eq(billingLedger.organizationId, orgId), eq(billingLedger.kind, kind as any)))
    : await db
        .select({ id: billingLedger.id })
        .from(billingLedger)
...
describe('BillingService.settle', () => {
  test('actual < estimated refunds the difference', async () => {
    ...
    const settled = await billing.settle({ reservationId: res.id, actualCredits: 70n })
...
describe('withBilling', () => {
  test('happy path: estimate → execute → settle', async () => {
    ...
    const billable = makeBillable<{ value: number }, string>({
      ...
      estimateCost: () => 100n,
      execute: async (input) => ({ output: `hello-${input.value}`, actualCredits: 80n }),
    })
  ...
  test('refunds and rethrows on execute error', async () => {
    ...
    const callable = withBilling(billable, billing)
    await expect(callable(undefined as void, { organizationId: org })).rejects.toBe(boom)
    expect(await billing.getBalance(org)).toBe(500n)
  })
  ...
  test('refunds and throws BillingTimeoutError on timeout', async () => {
    ...
    const callable = withBilling(billable, billing)
    await expect(callable(undefined as void, { organizationId: org })).rejects.toBeInstanceOf(
      BillingTimeoutError,
    )
    expect(await billing.getBalance(org)).toBe(500n)
  })
...
describe('withBillingAsync', () => {
  test('reserve → submit → settle later transitions reserved → settled', async () => {
    ...
    await billing.settle({ reservationId: handle.reservationId, actualCredits: 75n })
  ...
  test('G1 guardrail: ttlMs < timeoutMs * 2 throws BillableConfigError at first call', async () => {
    ...
    expect(() => withBillingAsync(badBillable, billing)).toThrow(BillableConfigError)
  })
})
...
describe('GET /billing/balance', () => {
  test('returns 401 when unauthenticated', async () => {
    ...
    const res = await app.handle(new Request('http://localhost/billing/balance'))
