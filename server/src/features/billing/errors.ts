...
export class InsufficientCreditsError extends BillingError {
  ...
  constructor(
    ...
    super(`Insufficient credits for organization ${organizationId}: requested ${requested}, available ${available}`)
  }
}
...
export class ReservationExpiredError extends BillingError {
  readonly status = 409
  constructor(public readonly reservationId: string, public readonly status_: string) {
    super(`Reservation ${reservationId} is no longer reservable (status=${status_})`)
  }
}
