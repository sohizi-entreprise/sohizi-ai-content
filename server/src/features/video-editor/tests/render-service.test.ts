...
let composition: { id: string; projectId: string; version: number } | null = null
...
mock.module('../repo', () => ({
  getCompositionById: async (id: string) => (composition?.id === id ? composition : null),
...
mock.module('../render-client', () => ({
  ...
  createRemoteRender: async (input: { jobId: string }) => {
    ...
    const job = remote({ jobId: input.jobId, status: 'queued', progress: 0, renderedFrames: 0 })
...
function snapshot(url = 'https://cdn.sohizi.test/videos/a.mp4'): CreateRenderInput {
  return {
    contractVersion: 1,
    fileName: 'export.mp4',
    composition: {
...
const create = () => renderService.createRender(COMPOSITION_ID, PROJECT_ID, USER_ID, snapshot())
...
describe('createRender', () => {
  ...
  test('rejects a composition from another project', async () => {
    composition = { id: COMPOSITION_ID, projectId: OTHER_PROJECT_ID, version: 1 }
  ...
  test('returns the in-flight job instead of starting a second render', async () => {
    ...
    remoteJobs.set(first.id, remote({ jobId: first.id, status: 'rendering', progress: 0.42 }))
  ...
  test('starts a new render once the previous one finished', async () => {
    ...
    remoteJobs.set(
      first.id,
      remote({ jobId: first.id, status: 'completed', progress: 1, outputKey: 'renders/a.mp4' }),
    )
  ...
  test('marks the job failed when submission is rejected', async () => {
    createRemoteRenderError = new BadRequest('This composition cannot be rendered')
...
describe('getRender', () => {
  test('is scoped to the project that owns the job', async () => {
    ...
    await expect(renderService.getRender(job.id, OTHER_PROJECT_ID)).rejects.toBeInstanceOf(NotFound)
  })
  ...
  test('stores the output location when the render completes', async () => {
    ...
    expect(jobs.get(job.id)?.outputKey).toBe(`renders/${PROJECT_ID}/${job.id}.mp4`)
  })
  ...
  test('surfaces a sanitized failure', async () => {
    ...
    expect(status.error).toEqual({ code: 'container_error', message: 'Render crashed' })
  })
  ...
  test('does not poll a job that already reached a terminal state', async () => {
    const job = buildJob({ status: 'completed', progress: 100, outputKey: 'renders/a.mp4' })
    jobs.set(job.id, job)
...
describe('cancelRender', () => {
  ...
  test('refuses to cancel a completed render', async () => {
    ...
    await expect(renderService.cancelRender(job.id, PROJECT_ID)).rejects.toBeInstanceOf(Conflict)
  })
})
...
describe('getRenderDownload', () => {
  ...
  test('refuses to sign an unfinished render', async () => {
    ...
    await expect(renderService.getRenderDownload(job.id, PROJECT_ID)).rejects.toBeInstanceOf(
      Conflict,
    )
  })
})
