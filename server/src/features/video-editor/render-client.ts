...
import { RENDER_CONTRACT_VERSION, type RenderComposition } from './render-schema';
...
export type RemoteRenderStatus = 'queued' | 'rendering' | 'completed' | 'failed' | 'cancelled';
...
async function readError(response: Response): Promise<ServiceError> {
  ...
  return { code: 'render_service_error', message: `Render service returned ${response.status}` };
}
...
function asJob(payload: unknown): RemoteRenderJob {
  ...
  if (!job || typeof job.jobId !== 'string') {
    throw new InternalServerError('Render service returned an unexpected payload');
  }
  return job;
}
...
export async function createRemoteRender(input: {
  ...
  if (response.status === 400 || response.status === 413) {
    throw new BadRequest(`This composition cannot be rendered: ${error.message}`);
  }
...
export async function getRemoteRender(
  ...
  const response = await call(`/v1/renders/${jobId}?${query.toString()}`, { method: 'GET' });
...
export async function cancelRemoteRender(jobId: string, projectId: string): Promise<void> {
  const query = new URLSearchParams({ projectId });
  const response = await call(`/v1/renders/${jobId}?${query.toString()}`, { method: 'DELETE' });
