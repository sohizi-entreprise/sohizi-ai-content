...
export const createRender = async (
  ...
  try {
    ...
    await repo.updateRenderJob(jobId, {
      ...
      failureMessage: error instanceof Error ? error.message : 'Could not start the render',
      finishedAt: new Date(),
    });
    throw error;
  }
...
async function reconcileRenderJob(job: VideoRenderJob): Promise<VideoRenderJob> {
...
async function applyRemoteStatus(
  ...
  if (remote.status === 'cancelled') {
    return update(job, { status: 'cancelled', finishedAt: job.finishedAt ?? new Date() });
  }
...
async function loadJob(renderJobId: string, projectId: string): Promise<VideoRenderJob> {
...
function assertMediaHostsAllowed(composition: RenderComposition): void {
  ...
  for (const track of composition.tracks) {
    for (const clip of track.clips) {
      ...
      const isAllowed = allowed.some((entry) => hostname === entry || hostname.endsWith(`.${entry}`));
      if (!isAllowed) rejected.add(hostname);
    }
  }

  if (rejected.size > 0) {
    throw new BadRequest(`Media host is not allowed for rendering: ${[...rejected].join(', ')}`);
  }
}
...
function toView(job: VideoRenderJob): RenderJobView {
  return {
    ...
    error: job.failureMessage
      ? { code: job.failureCode ?? 'render_failed', message: job.failureMessage }
      : null,
