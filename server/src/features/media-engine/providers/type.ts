type RequestStatus = "completed" | "failed" | "cancelled" | "timeout" | "processing" | "created";
...
export interface MediaEngineProvider {
    submitRequest: (model:string, payload: SubmitPayload) => Promise<SubmitResponse>;
    getRequestData: (requestId: string) => Promise<GetRequestDataResponse>;
    cancelRequest: (requestId: string) => Promise<CancelResponse>;
    estimateRequestPrice: (model:string, payload: SubmitPayload) => Promise<EstimateRequestPriceResponse>;
}