...
export const finishTool = buildBaseTool({
    name: 'finish',
    description: 'Call this tool to signal that the user\'s request is completely fulfilled, permanently blocked or needs clarification/confirmation, and the execution loop should end. CRITICAL: You MUST call this tool exactly once. Do not call this if any subtask is unresolved.',
    inputSchema: z.object({
        reason: z.enum(['done', 'question', 'blocked']).describe('The reason for calling this tool. "done" means you solved the user\'s request successfully.  "question" means you needs clarification or confirmation from the user. "blocked" means you could not proceed because of a blocker.'),
        message: z.string().describe('A super concise message to the user. If blocked, explain why. If question, ask for clarification. If done, summarize the result.'),
    }),
    execute: async (_, {state}) => {
        state.finishRun()
