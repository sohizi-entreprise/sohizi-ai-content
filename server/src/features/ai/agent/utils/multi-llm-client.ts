...
export class MultiModalClient {
    ...
    async textToSpeech(
        ...
        const audio = responseFormat === 'pcm'
            ? pcmToWav(rawAudio, parsePcmSampleRate(response.headers.get('Content-Type')))
            : rawAudio;
        const generationId =
    ...
    async submitVideoGeneration(request: GenerateVideoRequest): Promise<VideoSubmissionResult> {
        const {
            model,
            prompt,
            duration,
    ...
    private async transcribeOpenRouter(
        ...
        const costUsd =
            response.usage?.cost
            ?? (seconds > 0 ? getTranscriptionCost(seconds) : 0);
...
export type BillableMultiModalOutput =
    ...
    | { kind: 'caption'; text: string; words: TranscriptionWord[]; costUsd: number }
        ...
        async estimateCost(input: BillableMultiModalInput): Promise<Credits> {
            switch (input.kind) {
                case 'tts': {
                    ...
                    const costUsd = (chars / 1_000_000) * 0.60;
...
async function fetchAudioAsBase64(
    ...
    if (!response.ok) {
        throw new Error(`Failed to fetch audio for transcription (${response.status})`);
    }
