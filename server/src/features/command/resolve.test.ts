...
describe('command resolve', () => {
  it('extracts command names from markdown', () => {
    expect(extractCommandNames('Please #[command=summarize] this text')).toEqual(['summarize'])
    expect(extractCommandNames('#[command=proofread] and #[command=translate]')).toEqual([
      'proofread',
      'translate',
    ])
  })
