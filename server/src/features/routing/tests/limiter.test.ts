...
describe('VendorLimiter', () => {
    test('caps inflight at maxConcurrency', async () => {
        const a = await limiter.acquire(vendor, 'r1', { ...limits, maxConcurrency: 2 })
        const b = await limiter.acquire(vendor, 'r2', { ...limits, maxConcurrency: 2 })
        const c = await limiter.acquire(vendor, 'r3', { ...limits, maxConcurrency: 2 })
    ...
    test('same requestId replay does not consume another token or slot', async () => {
        const first = await limiter.acquire(vendor, 'same', { ...limits, burst: 5, rpm: 5 })
        expect(first.ok).toBe(true)
        const before = await limiter.tokenBucket(vendor)
        const second = await limiter.acquire(vendor, 'same', { ...limits, burst: 5, rpm: 5 })
    ...
    test('open circuit rejects other request ids until cooldown', async () => {
        ...
        await limiter.release(vendor, 'probe', { outcome: 'failure', retryAfterMs: 5_000, cooldownMs: 5_000 })
    ...
    test('after cooldown exactly one request is admitted as probe', async () => {
        expect((await limiter.acquire(vendor, 'seed', { ...limits, maxConcurrency: 50 })).ok).toBe(true)
        await limiter.release(vendor, 'seed', { outcome: 'failure', retryAfterMs: 50, cooldownMs: 50 })
        await Bun.sleep(200)
        ...
        expect(results.filter((r) => !r.ok && r.reason === 'circuit_open').length).toBe(19)
        const winner = results.findIndex((r) => r.ok)
        await limiter.release(vendor, `p${winner}`, { outcome: 'none', cooldownMs: 50 })
    })

    test('probe success closes the circuit', async () => {
        expect((await limiter.acquire(vendor, 'seed', { ...limits, maxConcurrency: 10 })).ok).toBe(true)
        await limiter.release(vendor, 'seed', { outcome: 'failure', retryAfterMs: 50, cooldownMs: 50 })
        await Bun.sleep(200)
        expect((await limiter.acquire(vendor, 'probe', { ...limits, maxConcurrency: 10 })).ok).toBe(true)
        await limiter.release(vendor, 'probe', { outcome: 'submit_ok', cooldownMs: 50 })
        const other = await limiter.acquire(vendor, 'other', { ...limits, maxConcurrency: 10 })
    ...
    test('expired inflight members are purged', async () => {
        ...
        const next = await limiter.acquire(vendor, 'fresh', { ...limits, maxConcurrency: 1 })
    ...
    test('redis errors fail closed', async () => {
        const broken = new VendorLimiter({
            redis: { eval: async () => { throw new Error('down') } } as never,
            keyPrefix: prefix,
        })
    ...
    test('20 parallel acquires honor maxConcurrency=5', async () => {
        const results = await Promise.all(
            Array.from({ length: 20 }, (_, i) =>
                limiter.acquire(vendor, `c${i}`, { ...limits, maxConcurrency: 5, burst: 50 }),
            ),
        )
        expect(results.filter((r) => r.ok).length).toBe(5)
        expect(await limiter.inflightCount(vendor)).toBe(5)
        await Promise.all(
            results.flatMap((r, i) => r.ok ? [limiter.release(vendor, `c${i}`, { outcome: 'none' })] : []),
        )
    })
})
