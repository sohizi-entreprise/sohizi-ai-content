...
beforeAll(async () => {
    ...
    const [fakeA] = await db.insert(llmVendors).values({
        name: FAKE_A,
        kind: 'media',
        enabled: true,
        rateLimit: DEFAULT_VENDOR_RATE_LIMIT,
    }).returning()
    const [fakeB] = await db.insert(llmVendors).values({
        name: FAKE_B,
        kind: 'media',
        enabled: true,
        rateLimit: DEFAULT_VENDOR_RATE_LIMIT,
    }).returning()
    const [unknown] = await db.insert(llmVendors).values({
        name: UNREGISTERED,
        kind: 'media',
        enabled: true,
        rateLimit: DEFAULT_VENDOR_RATE_LIMIT,
    }).returning()
    const [llm] = await db.insert(llmVendors).values({
        name: LLM_VENDOR,
        kind: 'llm',
        enabled: true,
        rateLimit: DEFAULT_VENDOR_RATE_LIMIT,
    ...
    await db.insert(llmVendorsAndModels).values([
        { vendorId: fakeA.id, modelId: MODEL, apiName: 'api-a', enabled: true, priority: 50 },
        { vendorId: fakeB.id, modelId: MODEL, apiName: 'api-b', enabled: true, priority: 10 },
        { vendorId: unknown.id, modelId: MODEL, apiName: 'api-x', enabled: true, priority: 1 },
        { vendorId: llm.id, modelId: MODEL, apiName: 'api-llm', enabled: true, priority: 1 },
    ])
})
...
describe('listMediaRoutes', () => {
    beforeEach(async () => {
        await db.update(llmModels).set({ enabled: true }).where(eq(llmModels.id, MODEL))
        await db.update(llmVendors).set({ enabled: true }).where(eq(llmVendors.id, ids.fakeA))
        await db.update(llmVendorsAndModels).set({ enabled: true }).where(eq(llmVendorsAndModels.vendorId, ids.fakeA))
    })
    ...
    test('skips disabled model', async () => {
        await db.update(llmModels).set({ enabled: false }).where(eq(llmModels.id, MODEL))
        expect(await listMediaRoutes(MODEL)).toEqual([])
    })

    test('skips disabled vendor', async () => {
        await db.update(llmVendors).set({ enabled: false }).where(eq(llmVendors.id, ids.fakeA))
        const names = (await listMediaRoutes(MODEL)).map((r) => r.vendorName)
        expect(names).toEqual([FAKE_B])
    })

    test('skips disabled binding', async () => {
        await db.update(llmVendorsAndModels).set({ enabled: false }).where(eq(llmVendorsAndModels.vendorId, ids.fakeA))
        const names = (await listMediaRoutes(MODEL)).map((r) => r.vendorName)
        expect(names).toEqual([FAKE_B])
    })
