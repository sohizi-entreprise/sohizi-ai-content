...
import { MediaRateLimitError, MediaValidationError } from '@/features/media-engine/errors'
import { VendorLimiter } from '../limiter'
import { pollWithSticky, resolveAndAcquire, submitWithFailover } from '../router'
...
const identityMap = async (_vendor: string, _model: string, payload: Record<string, unknown>) => payload
...
beforeAll(async () => {
    ...
    const [a] = await db.insert(llmVendors).values({
        name: FAKE_A,
        kind: 'media',
        enabled: true,
        rateLimit: { rpm: 60, burst: 60, maxConcurrency: 2 },
    }).returning()
    const [b] = await db.insert(llmVendors).values({
        name: FAKE_B,
        kind: 'media',
        enabled: true,
        rateLimit: { rpm: 60, burst: 60, maxConcurrency: 2 },
    ...
    await db.insert(llmVendorsAndModels).values([
        { vendorId: a.id, modelId: MODEL, apiName: 'api-a', enabled: true, priority: 10 },
        { vendorId: b.id, modelId: MODEL, apiName: 'api-b', enabled: true, priority: 20 },
    ])
})

afterAll(async () => {
    await db.delete(llmVendorsAndModels).where(eq(llmVendorsAndModels.modelId, MODEL))
...
beforeEach(async () => {
    ...
    limiter = new VendorLimiter({ redis, keyPrefix: prefix, acquireTimeoutMs: 500 })
...
const resolveOpts = (requestId: string, extra?: Record<string, unknown>) => ({
    ...
    leaseTtlMs: 60_000,
    limiter,
    mapPayload: identityMap as typeof import('@/features/media-engine/providers/utils').mapVendorPayload,
    createProvider,
    ...extra,
})
...
describe('resolveAndAcquire', () => {
    ...
    test('skips a saturated preferred vendor', async () => {
        expect((await limiter.acquire(FAKE_A, 'fill-1', { rpm: 60, burst: 60, maxConcurrency: 2, leaseTtlMs: 60_000 })).ok).toBe(true)
        expect((await limiter.acquire(FAKE_A, 'fill-2', { rpm: 60, burst: 60, maxConcurrency: 2, leaseTtlMs: 60_000 })).ok).toBe(true)
    ...
    test('skips a circuit-open preferred vendor', async () => {
        expect((await limiter.acquire(FAKE_A, 'seed', { rpm: 60, burst: 60, maxConcurrency: 2, leaseTtlMs: 60_000 })).ok).toBe(true)
        await limiter.release(FAKE_A, 'seed', { outcome: 'failure', retryAfterMs: 10_000, cooldownMs: 10_000 })
    ...
    test('exclude skips a vendor', async () => {
        const decision = await resolveAndAcquire(resolveOpts('r-ex', { exclude: [FAKE_A] }))
        expect(decision.vendorName).toBe(FAKE_B)
        await limiter.release(FAKE_B, 'r-ex', { outcome: 'none' })
    })

    test('throws NoRouteError when the model has no media routes', async () => {
        await expect(resolveAndAcquire(resolveOpts('r-none', { modelId: `${PREFIX}-missing` }))).rejects.toBeInstanceOf(NoRouteError)
    })

    test('throws AllVendorsSaturatedError when every vendor rejects', async () => {
        expect((await limiter.acquire(FAKE_A, 'a1', { rpm: 60, burst: 60, maxConcurrency: 2, leaseTtlMs: 60_000 })).ok).toBe(true)
        expect((await limiter.acquire(FAKE_A, 'a2', { rpm: 60, burst: 60, maxConcurrency: 2, leaseTtlMs: 60_000 })).ok).toBe(true)
        expect((await limiter.acquire(FAKE_B, 'b1', { rpm: 60, burst: 60, maxConcurrency: 2, leaseTtlMs: 60_000 })).ok).toBe(true)
        expect((await limiter.acquire(FAKE_B, 'b2', { rpm: 60, burst: 60, maxConcurrency: 2, leaseTtlMs: 60_000 })).ok).toBe(true)
        await expect(resolveAndAcquire(resolveOpts('r-sat'))).rejects.toBeInstanceOf(AllVendorsSaturatedError)
...
describe('submitWithFailover', () => {
    test('fails over on rate limit to the next vendor', async () => {
        setFakeScript(FAKE_A, { submitError: new MediaRateLimitError('429', undefined, 1000) })
    ...
    test('does not fail over on validation errors and releases the slot', async () => {
        setFakeScript(FAKE_A, { submitError: new MediaValidationError('bad payload') })
        await expect(submitWithFailover(resolveOpts('r-val'))).rejects.toBeInstanceOf(MediaValidationError)
