...
import { DEFAULT_VENDOR_CIRCUIT_CONFIG, DEFAULT_VENDOR_RATE_LIMIT } from '@/type'
import type { AcquireResult, ReleaseOutcome } from './types'

const ACQUIRE_LUA = readFileSync(join(import.meta.dir, 'lua/acquire.lua'), 'utf8')
const RELEASE_LUA = readFileSync(join(import.meta.dir, 'lua/release.lua'), 'utf8')
...
function parseEvalTuple(raw: unknown): AcquireResult {
    ...
    if (reason === 'circuit_open' || reason === 'concurrency' || reason === 'rpm') {
...
export class VendorLimiter {
    ...
    async acquire(vendorName: string, requestId: string, limits: LimiterAcquireLimits): Promise<AcquireResult> {
        const keys = this.keys(vendorName)
        const burst = limits.burst > 0 ? limits.burst : limits.rpm
        const probeTtlMs = limits.probeTtlMs ?? DEFAULT_VENDOR_CIRCUIT_CONFIG.probeTtlMs
        ...
        try {
            ...
            if (parsed.ok) {
                await this.redis.set(this.ownerKey(requestId), vendorName, 'PX', limits.leaseTtlMs)
            }
            return parsed
        } catch (error) {
    ...
    async release(vendorName: string, requestId: string, options: LimiterReleaseOptions): Promise<void> {
        const keys = this.keys(vendorName)
        const retryAfterMs = options.retryAfterMs ?? 0
        const cooldownMs = options.cooldownMs ?? DEFAULT_VENDOR_CIRCUIT_CONFIG.cooldownMs
        try {
    ...
    async releaseByRequestId(requestId: string, options: LimiterReleaseOptions): Promise<void> {
    ...
    async tokenBucket(vendorName: string): Promise<{ tokens: number | null; ts: number | null }> {
        const [tokens, ts] = await this.redis.hmget(this.keys(vendorName).tb, 'tokens', 'ts')
        return {
