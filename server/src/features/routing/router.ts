...
import type { GetRequestDataResponse, SubmitPayload } from '@/features/media-engine/providers/type'
...
function isFailoverError(error: unknown): boolean {
    return error instanceof MediaRateLimitError || error instanceof MediaServiceUnavailableError
}

export async function resolveAndAcquire(options: ResolveOptions): Promise<RouteDecision> {
    const vendorLimiter = options.limiter ?? defaultLimiter
    const mapPayload = options.mapPayload ?? mapVendorPayload
    const exclude = new Set(options.exclude ?? [])

    const routes = (await listMediaRoutes(options.modelId))
        .filter((route) => !exclude.has(route.vendorName))
    ...
    for (const route of routes) {
        const slot = await vendorLimiter.acquire(route.vendorName, options.requestId, {
            rpm: route.rpm,
            burst: route.burst,
        ...
        const mappedPayload = await mapPayload(route.vendorName, options.modelId, options.payload)
        return {
...
export async function submitWithFailover(options: SubmitWithFailoverOptions): Promise<StickyDecision> {
    const vendorLimiter = options.limiter ?? defaultLimiter
