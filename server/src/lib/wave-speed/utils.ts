...
type ErrorCode = 'BAD_REQUEST' | 'UNAUTHORIZED' | 'TOO_MANY_REQUESTS' | 'INTERNAL_SERVER_ERROR' | 'ACCOUNT_ISSUE' | 'GENERATION_FAILED' | 'REQUEST_TIMEOUT' | 'UNKNOWN';
...
type GetJobResultResponse = {
    status: "completed" | "failed" | "cancelled" | "timeout" | "processing" | "created";
    outputs: any;
}

const buildHeader = (apiKey: string, authOnly: boolean = false): HeadersInit => {
    if(authOnly) {
        return {
...
export const submitJob = async(apiKey: string, modelId: string, input: any) => {
...
export const getJobResult = async(apiKey: string, jobId: string) => {
    ...
    const result = await requestJson(url, options) as {data: GetJobResultResponse};
    const status = result.data.status;

    if (["failed", "cancelled", "timeout"].includes(status)) throw new WaveSpeedError(500, JSON.stringify(result.data), 'GENERATION_FAILED');
    if (!["created", "processing", "completed"].includes(status)) throw new WaveSpeedError(500, "Unexpected status: " + status, 'UNKNOWN');
...
export const deleteJob = async(apiKey: string, jobIds: string[]) => {
    ...
    const result = await requestJson(url, options) as {data: {deleted_count: number}};
    return result.data.deleted_count;
}

export const getInferencePrice = async(apiKey: string, modelId: string, input: any) => {
    ...
    const result = await requestJson(url, options) as {data: {price: number; currency: "USD"}};
    return {
