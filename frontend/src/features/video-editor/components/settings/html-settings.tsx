...
import { isStringVariable, isColorVariable, isNumberVariable, isBooleanVariable, isEnumVariable } from '@hyperframes/core'
...
export const HtmlSettings: React.FC<{ clip: HtmlClip }> = ({ clip }) => {
        ...
        {clip.variables.map(v => (
          <label key={v.id}>   {/* ← v.id */}
            ...
            <VariableInput
              ...
              onChange={value => updateClip(clip.id, { values: { ...clip.values, [v.id]: value } })}
            />
          </label>
        ))}
      </div>
  ...
  const VariableInput: React.FC<{
    ...
    if (isNumberVariable(v))
      return <input type="range" min={v.min ?? 0} max={v.max ?? 100}
        value={value as number} onChange={e => onChange(parseFloat(e.target.value))} />
    if (isBooleanVariable(v))
      return <input type="checkbox" checked={value as boolean} onChange={e => onChange(e.target.checked)} />
    if (isEnumVariable(v))
      return <select value={value as string} onChange={e => onChange(e.target.value)}>
