...
export function ModelCategoriesPage() {
  const { data: categories = [], isLoading, error } = useQuery(listAdminCategoriesQueryOptions())
  ...
  const handleDelete = (category: AdminCategory) => {
    const assignmentNote =
      category.modelCount > 0
        ...
        : ''
    if (!window.confirm(`Delete category “${category.name}”?${assignmentNote} This cannot be undone.`)) {
      return
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Model categories</h1>
