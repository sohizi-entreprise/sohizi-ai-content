...
function refineLineReplacement(
  ...
  if (
    getSimilarity(changes as LibraryChange[], removedText, insertedText) <
  ...
  return compactParts(
    (changes as LibraryChange[]).map(changeToPart),
  );
}
...
function getSimilarity(
  ...
  const unchangedSignalLength = changes.reduce(
    (total, change) => {
 ...
 */
export function parseDiffMarkdown(
  markdown: string,
): DiffPart[] {
  const parts: DiffPart[] = [];
  let equalBuffer = '';
...
function readMarkedPayload(
  ...
  throw new DiffSyntaxError(
    `Unclosed ${kind} marker`,
    startIndex - 2,
  );
}
...
function getReservedSequenceAt(
  ...
  return RESERVED_SEQUENCES.find((sequence) =>
...
export function acceptDiffMarkdown(
  markdown: DiffMarkdown,
): string {
  return resolveDiffParts(
    parseDiffMarkdown(markdown),
    'accept',
  );
}

export function rejectDiffMarkdown(
  markdown: DiffMarkdown,
): string {
  return resolveDiffParts(
    parseDiffMarkdown(markdown),
 ...
 */
export function asDiffMarkdown(
  markdown: string,
): DiffMarkdown {
  parseDiffMarkdown(markdown);
...
function splitLinesPreservingEndings(
  text: string,
): string[] {
  const lines: string[] = [];
  let start = 0;
...
function resolveOptions(
  ...
  return {
    ...
    maxWordRefinementLines: positiveInteger(
      options.maxWordRefinementLines,
      12,
    ),
    minWordSimilarity: clamp(
      options.minWordSimilarity ?? 0.2,
      0,
      1,
    ),
  };
