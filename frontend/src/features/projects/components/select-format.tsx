...
import type { ProjectFormatOption } from '../type'

type Props = {
  data: Array<ProjectFormatOption>
}

const SelectFormat = ({ data }: Props) => {
  const { format, setFormat } = useNewProjectStore()
  return (
...
function FormatItem({
  ...
  return (
    <div className="flex flex-col gap-1 items-center relative group">
      <div
        className="size-20 rounded-full hover:scale-110 transition-all flex items-center justify-center bg-white/10"
        onClick={onClick}
      >
