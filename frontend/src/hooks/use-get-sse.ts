...
export type SseEventHandler = (
  data: unknown,
  options: { event: MessageEvent; closeEventSource: () => void },
) => void
...
export const useGetSSE = <TEvent extends string>(
  params: GetSSEOptions<TEvent>,
) => {
  ...
  const subscribe = useCallback((sseUrl: string) => {
    ...
    const handleEvent = (eventName: TEvent) => (event: MessageEvent) => {
      try {
        ...
        const err =
          e instanceof Error ? e : new Error('Failed to parse stream event')
        onError?.(err)
      }
    }
