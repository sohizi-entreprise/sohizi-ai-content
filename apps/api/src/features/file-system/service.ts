...
export const createFileNode = async (data: FileCreationRequest) => {
  try {
    ...
    if (
      error instanceof FileSystemConflictError ||
      error instanceof FileSystemInputError
    ) {
...
export const deleteFileNode = async (projectId: string, fileId: string) => {
  try {
    ...
    const isAssetFile =
      !fileNode.directory && assetFormats.includes(fileNode.format!)
...
export const updateFileContent = async (
  projectId: string,
  fileNodeId: string,
  data: UpdateTextFileContentRequest,
) => {
  try {
    if (data.diff) {
...
export const listFileTreePerLevel = async (
  projectId: string,
  parentId: string,
) => {
...
export const getFileContent = async (
  ...
  switch (fileNode.format) {
    case fileFormat.MARKDOWN: {
      const textContent = await fileSystemRepo.getFileContentById(
        projectId,
        fileNodeId,
      )
      if (!textContent) {
    ...
    case fileFormat.AI_GENERATED: {
      const aiGeneratedAssets = await mediaRepo.listAiGeneratedAssets(
        projectId,
        paginationOptions,
      )
      return {
...
export const searchFilesByName = async (request: {
  projectId: string
  name: string
  limit?: number
  directory?: boolean
  ...
  await validateProject(request.projectId)
  return fileSystemRepo.searchFileNodesByName(
    request.projectId,
    request.name,
    request.limit ?? 15,
    {
...
export const listFilesByFormat = async (request: {
  projectId: string
  format: FileFormat
  limit?: number
}) => {
...
export const listAssetsFolder = async (request: {
  projectId: string
  name?: string
  format?: FileFormat
  limit?: number
...
export const getPendingFileOperation = async (
  projectId: string,
  fileNodeId: string,
) => {
  await validateProject(projectId)
  const pendingOperation =
    await fileSystemRepo.getPendingFileOperation(fileNodeId)
  return {
    operation: pendingOperation,
  }
}

export const deletePendingFileOperation = async (
  projectId: string,
  fileNodeId: string,
) => {
  await validateProject(projectId)
