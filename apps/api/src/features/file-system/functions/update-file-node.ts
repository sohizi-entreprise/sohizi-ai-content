...
export const updateFileNode = async (
  ...
  const currentFileNode = await fileSystemRepo.getFileNodeById(
    projectId,
    request.id,
  )
  if (!currentFileNode) {
  ...
  const nextParentId =
    request.parentId === undefined ? currentFileNode.parentId : request.parentId
  const shouldReorder =
    request.position !== undefined ||
    request.anchorId !== undefined ||
    nextParentId !== currentFileNode.parentId
