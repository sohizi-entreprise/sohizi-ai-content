...
export async function validateMoveTarget(
  ...
  if (parentId !== null) {
    const destinationParent = await fileSystemRepo.getFileNodeById(
      projectId,
      parentId,
    )
  ...
  const wouldCreateCycle = await wouldCreateFileNodeCycle(
    projectId,
    fileNode.id,
    parentId,
  )
  if (wouldCreateCycle) {
    throw new FileSystemInputError(
      `Cannot assign parent "${parentId}" because it would create a cycle.`,
    )
  }
  ...
  if ((fileNode.parentId ?? null) !== parentId) {
    ...
    if (destinationSiblings.length >= MAX_FILE_IN_DIRECTORY) {
      throw new FileSystemInputError(
        `Maximum file count of ${MAX_FILE_IN_DIRECTORY} exceeded in this directory`,
      )
    }
  }
  ...
  if (nextDepth > MAX_FILE_DEPTH) {
    throw new FileSystemInputError(
      `Maximum file depth of ${MAX_FILE_DEPTH} exceeded`,
    )
  }
}
