...
export class ContextManager {
        ...
        if (
          ...
          return {
            ...
            output: {
              ...output,
              value:
                output.value.slice(0, this.pruneMaxLength) + TRUNCATION_MARKER,
            },
          }
        }
        return part
  ...
  async summarizeMessages(
    ...
    const lastStart = this.adjustLastBoundary(
      conversation,
      conversation.length - this.keepLast,
    )
  ...
  private async getSummaryStream(): Promise<BilledLlmStream | null> {
    ...
    const model = await this.session.resolveModel(
      this.summaryModelId,
      this.vendor,
    )
    if (!model) {
