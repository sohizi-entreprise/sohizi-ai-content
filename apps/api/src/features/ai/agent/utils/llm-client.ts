 ...
 */
const passthroughOpenRouterMedia: Experimental_DownloadFunction = async (
  downloads,
) => {
  return Promise.all(
...
export class LlmClient {
  ...
  private async *fullResponse({
    ...
    try {
      ...
      const openRouterUsage = response.finalStep.providerMetadata?.openrouter
        ?.usage as { cost: number } | undefined
  ...
  private async *streamResponse({
    ...
    try {
      ...
      const [finish, finalStep] = await Promise.all([
        response.finishReason,
        response.finalStep,
      ])
      finishReason = finish
      const openRouterUsage = finalStep.providerMetadata?.openrouter?.usage as
 ...
 */
export type BillableLlmClient = Billable<BillableLlmInput, BillableLlmOutput> &
  BillableStream<BillableLlmInput, LlmChunk>

export function createBillableLlmClient(
  config: BillableLlmConfig,
): BillableLlmClient {
  ...
  return {
    ...
    async execute(
      input: BillableLlmInput,
      ctx: BillableContext,
    ): Promise<BillableResult<BillableLlmOutput>> {
