...
interface BaseToolDefinition<T extends z.ZodSchema> {
  ...
  ) => Promise<ToolResult> | AsyncGenerator<AgenticToolResult, void, unknown>
}
...
export class BaseTool<T extends z.ZodSchema> {
  ...
  validateInput(input: z.infer<T>): z.core.output<T> {
    ...
    if (!validated.success) {
      throw new Error(
        `Your input is invalid: ${validated.error.message}. The expected input is:\n ${JSON.stringify(toJSONSchema(this.params.inputSchema))}.`,
      )
    }
...
export class LlmTool<T extends z.ZodSchema> extends BaseTool<T> {
  ...
  async *execute(
    toolCall: ToolCall,
    options: ToolExecutionOptions,
  ): AsyncGenerator<ToolResultComplete, void, unknown> {
    ...
    const model = await session.resolveModel(
      this.config.modelId,
      DEFAULT_AGENT_VENDOR,
    )
    if (!model) {
      throw new Error(
        `Model ${this.config.modelId} not found in the current session`,
      )
    }
...
export const buildBaseTool = <T extends z.ZodSchema>(
  params: BaseToolDefinition<T>,
): BaseTool<T> => {
  return new BaseTool(params)
}

export const buildLlmTool = <T extends z.ZodSchema>(
  params: LlmToolParams<T>,
): LlmTool<T> => {
