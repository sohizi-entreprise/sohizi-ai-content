...
async function executeUpdateComposition(
  input: z.infer<typeof updateCompositionCommand>,
) {
...
async function executeAddTrack(input: z.infer<typeof addTrackCommand>) {
  ...
  return success(
    `Track created: ID: ${track.id} - Type: ${track.type} - Position: ${track.position}`,
  )
}
...
async function executeAddClip(input: z.infer<typeof addClipCommand>) {
  ...
  return success(
    `Clip created: ${clip.id} [${clip.type}] at frames ${clip.startFrame}–${clip.endFrame}`,
  )
}
...
async function executeBatch(input: z.infer<typeof batchCommand>) {
  ...
  try {
    ...
    return success(
      `Batch executed: ${results.length} operations completed atomically.`,
    )
  } catch (error) {
