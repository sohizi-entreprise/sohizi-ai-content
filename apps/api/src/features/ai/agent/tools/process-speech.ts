...
const processAiAudio = withBilling(multimodalBillable, billingService)
...
export const processSpeechTool = buildBaseTool({
  ...
  execute: async ({ command }, { session }) => {
    switch (command.cmd) {
      ...
      default:
        return failure(
          'Invalid command. Supported commands are "text-to-speech" and "speech-to-text".',
        )
    }
  },
})
...
async function textToSpeech(
  command: z.infer<typeof textToSpeechInputSchema>,
  session: Session,
): Promise<ToolResult> {
  try {
    ...
    return success(
      `The audio file is saved in the file [ID ${fileNode.id}] [Format ${fileNode.format}].`,
    )
  } catch {
    return failure(`Text to speech conversion failed.`)
  }
...
async function speechToText(
  command: z.infer<typeof speechToTextInputSchema>,
  session: Session,
): Promise<ToolResult> {
  try {
    ...
    if (command.writeToFolderId) {
      if (!command.fileName) {
        return failure(
          `File name is required when writing to writeToFolderId is specified.`,
        )
      }
      const folder = await session.resolveFileByPathOrId(
        command.writeToFolderId,
      )
      if (!folder) {
      ...
      return success(
        `Speech to text conversion completed successfully. File [ID ${fileNode.id}] [Format ${fileNode.format}] was successfully created.`,
      )
    }
