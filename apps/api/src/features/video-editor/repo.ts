...
export const createComposition = async (data: CreateCompositionData) => {
  const result = await db.insert(videoCompositions).values(data).returning()
  return result[0]
}
...
export const getTracksByCompositionId = async (
    ...
    .where(
      and(
        ...
        minPosition !== undefined
          ? gte(videoTracks.position, minPosition)
          : undefined,
        maxPosition !== undefined
          ? lte(videoTracks.position, maxPosition)
          : undefined,
      ),
    )
...
export const createTrack = async (data: CreateTrackData) => {
  const result = await db.insert(videoTracks).values(data).returning()
  return result[0]
}
...
export const getClipsByFilters = async (
  compositionId: string,
  filters: ClipFilters,
) => {
...
export const createClip = async (data: CreateClipData) => {
  const result = await db.insert(videoClips).values(data).returning()
  return result[0]
}
...
export const getActiveRenderJobForComposition = async (
  compositionId: string,
) => {
  const result = await db
    .select()
...
export const listRenderJobsForComposition = async (
  compositionId: string,
  limit = 10,
) => {
  return db
...
export const executeBatch = async (operations: BatchOperation[]) => {
  const sorted = [...operations].sort((a, b) => OP_ORDER[a.op] - OP_ORDER[b.op])
