...
export const listEnabledModelsByCategories = async (categories: string[]) => {
  ...
  return db
    ...
    .innerJoin(
      modelsAndCategories,
      eq(llmModels.id, modelsAndCategories.modelId),
    )
    .innerJoin(
      modelCategories,
      eq(modelsAndCategories.categoryId, modelCategories.id),
    )
    .where(
      and(
        inArray(modelCategories.name, categories),
...
export const getModelWithRelations = async (id: string) => {
  const rows = await db
    ...
    .leftJoin(
      llmVendorsAndModels,
      eq(llmVendorsAndModels.modelId, llmModels.id),
    )
...
export const listModelParameterBindings = async (modelId: string) => {
  const rows = await db
    ...
    .leftJoin(
      modelsAndParameters,
      eq(modelsAndParameters.modelId, llmModels.id),
    )
    .leftJoin(
      modelParameters,
      eq(modelsAndParameters.parameterId, modelParameters.id),
    )
    .leftJoin(
      modelsAndParameterOptions,
      and(
...
export const replaceModelCategories = async (
  ...
  await db
    .delete(modelsAndCategories)
    .where(eq(modelsAndCategories.modelId, modelId))
...
export const updateParameterOption = async (
  ...
  const [updated] = await db
    ...
    .where(
      and(
        eq(parameterOptions.id, optionId),
        eq(parameterOptions.parameterId, parameterId),
      ),
    )
    .returning()
  return updated
...
export const upsertVendorParameterMapping = async (data: {
  ...
  const [row] = await db
    ...
    .onConflictDoUpdate({
      target: [
        llmVendorsAndParameters.vendorId,
  ...
  return db.transaction(async (tx) => {
    ...
    const optionIds = [
      ...new Set(
        bindings.flatMap((binding) =>
          (binding.options ?? []).map((option) => option.optionId),
        ),
      ),
    ]
    ...
    if (parameterIds.length > 0) {
      const ownershipRows = await tx
        ...
        .leftJoin(
          ...
          and(
            eq(parameterOptions.parameterId, modelParameters.id),
            optionIds.length > 0
              ? inArray(parameterOptions.id, optionIds)
              : sql`false`,
          ),
        )
        .where(inArray(modelParameters.id, parameterIds))

      const foundParameterIds = new Set(
      ...
      const requestedPairs = bindings.flatMap((binding) =>
        (binding.options ?? []).map(
          (option) => `${binding.parameterId}:${option.optionId}`,
        ),
      )
...
export const listVendors = async () => {
  const rows = await db
    ...
    .leftJoin(
      llmVendorsAndModels,
      eq(llmVendorsAndModels.vendorId, llmVendors.id),
    )
...
export const updateModelVendorBinding = async (
  ...
  const [updated] = await db
    ...
    .where(
      and(
        eq(llmVendorsAndModels.modelId, modelId),
        eq(llmVendorsAndModels.vendorId, vendorId),
      ),
    )
    .returning()
  return updated
