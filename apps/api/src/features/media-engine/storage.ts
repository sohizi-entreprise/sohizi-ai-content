...
function createSignedObjectUrl(
  ...
  const { url, canonicalUri } = buildObjectUploadUrl(
    config.endpoint,
    config.bucket,
    storageKey,
  )
...
function buildCanonicalQueryString(params: Record<string, string>): string {
  return Object.entries(params)
    .map(
      ([key, value]) =>
        [awsEncodeURIComponent(key), awsEncodeURIComponent(value)] as const,
    )
...
export async function uploadFromUrl(
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to download from ${sourceUrl}: ${response.statusText}`,
    )
  }
...
export async function uploadGeneratedMedia(sourceUrl: string): Promise<{
  ...
  if (sourceUrl.startsWith('data:')) {
    ...
    const destPath = buildStoragePath(
      folder,
      fileNameFromSource(sourceUrl, folder),
    )
