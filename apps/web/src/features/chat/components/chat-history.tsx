...
export function ChatHistory({ projectId }: { projectId: string }) {
  const { data: conversations = [], isLoading } = useInfiniteQuery(
    listConversationsQueryOptions(projectId),
  )
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="right" showCloseButton={false} className="w-2xl">
          ...
          {isLoading ? (
            ...
            conversations.map((conv) => (
              <ConversationItem
                key={conv.id}
                conversation={conv}
                onSelect={handleOnConversationChange}
                projectId={projectId}
              />
            ))
          )}
        ...
        <SheetClose asChild>
          <Button
            size="icon"
            className="size-8 rounded-full absolute top-4 -left-10"
            aria-label="Close"
          >
...
function ConversationItem({
  ...
  const { mutate: deleteConversation, isPending } = useMutation(
    deleteConversationMutationOptions(projectId),
  )
  ...
  return (
    ...
    >
      <div className="flex-1 min-w-0">
        <div className="truncate text-left min-w-0">{conversation.title}</div>
