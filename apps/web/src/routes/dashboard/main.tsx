    ...
    <div className="min-h-screen bg-background flex">
      ...
      <aside className="w-64 bg-background/80 border-r border-white/5 backdrop-blur-xl flex flex-col px-4 pb-8">
        <div className="h-20 flex items-center px-6">
          ...
          <span className="hidden lg:block text-2xl font-bold tracking-tighter ml-3">
            Sohizi<span className="text-primary italic">AI</span>
          </span>
        </div>
        {/* Nav bar items */}
        <nav className="flex-1 overflow-y-auto space-y-2 py-8 font-space-grotesk">
          {links.map((link) => (
            <Link
              ...
              to={link.href}
              className={cn(
                "flex items-center gap-4 px-4 py-3 font-medium tracking-wide rounded-xl hover:bg-accent",
        ...
        <div className="mt-4 flex items-center gap-3 p-2 rounded-xl hover:bg-white/5 cursor-pointer transition-all border border-transparent hover:border-white/5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] text-primary/70 uppercase font-bold">
              Neural Plan
            </p>
          </div>
