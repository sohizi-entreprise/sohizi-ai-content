...
export function buildVariableOverride(
  variables: Array<CompositionVariable>,
  values: Record<string, string | number | boolean>,
): string {
  const cssLines: Array<string> = []
      ...
      if (depth === 0) {
        try {
          ...
          return Array.isArray(parsed)
            ? (parsed as Array<CompositionVariable>)
            : []
        } catch {
          return []
        }
