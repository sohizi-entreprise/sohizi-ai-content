...
> {
    ...
    for (let poll = 0; poll < maxPolls; poll++) {
      const status = await step.do(
        ...
        async () => {
          ...
          if (!response.ok) {
            throw await this.toStepError(response, 'Container status check failed')
          }
      ...
      if (status.state === 'failed') {
        const message = status.error?.message ?? 'Render failed inside container'
  ...
  private async toStepError(response: Response, summary: string): Promise<Error> {
