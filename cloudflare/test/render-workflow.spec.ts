...
function runWorkflow(env: unknown) {
  const instance = new RenderWorkflow(
    {} as never,
    env as never,
  )
  const { step, names, sleeps } = createStep()
  const promise = instance.run(
    { payload: PARAMS, timestamp: new Date(), instanceId: PARAMS.jobId, workflowName: 'test' },
    step as never,
  )
...
describe('RenderWorkflow', () => {
  ...
  it('starts the render, publishes progress and stores the output in R2', async () => {
    ...
    containerFetch.mockImplementation(async (url, init) => {
      ...
      if (init.method === 'GET') {
        ...
        return jsonResponse(
          statusCalls === 1
            ? statusBody()
            : statusBody({ state: 'completed', progress: 1, renderedFrames: 90 }),
        )
      }
    ...
    expect(bucket.objects.get(PARAMS.outputKey)?.byteLength).toBe(video.byteLength)
  ...
  it('sends the snapshot as a buffered string so the subrequest has a known length', async () => {
    const { bucket, env } = createTestEnv()
    const snapshot = JSON.stringify({ jobId: PARAMS.jobId, composition: { fps: 30 } })
    ...
    containerFetch.mockImplementation(async (_url, init) => {
      ...
      return jsonResponse(
        statusBody({
          state: 'failed',
          error: { code: 'render_failed', message: 'stop here', retryable: false },
        }),
      )
    })
  ...
  it('fails without retrying when the container reports a permanent error', async () => {
    ...
    containerFetch.mockImplementation(async (_url, init) => {
      ...
      return jsonResponse(
        statusBody({
          state: 'failed',
          error: { code: 'render_failed', message: 'Asset 404', retryable: false },
        }),
      )
    })
