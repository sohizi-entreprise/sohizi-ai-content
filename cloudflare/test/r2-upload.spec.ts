...
describe('uploadStreamToR2', () => {
  ...
  it('splits a large render into parts', async () => {
    ...
    expect(bucket.objects.get('renders/p/j.mp4')?.byteLength).toBe(15 * 1024 * 1024)
  })
  ...
  it('aborts the multipart upload when the source stream fails', async () => {
    ...
    await expect(
      uploadStreamToR2(bucket as unknown as R2Bucket, 'renders/p/j.mp4', failing, {
        contentType: 'video/mp4',
      }),
  ...
  it('rejects an empty render instead of storing a broken file', async () => {
    ...
    await expect(
      uploadStreamToR2(bucket as unknown as R2Bucket, 'renders/p/j.mp4', streamOf([]), {
        contentType: 'video/mp4',
      }),
